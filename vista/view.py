...
class View(Model):
    ...
    def main_sup_panel(self, id_sup : str, name : str):
        ...
        ttk.Button(self.window, text="Salir", command = self.label_main).grid(row=4, column=3)
        # bienes
        ...
        ttk.Button(self.window, text="Generar registro de bienes", command = lambda : self.generar_excel(id_area)).grid(row=4, column=2)
        # pie de pagina
        self.pie_pagina(0, 3)
    ...
    def topLevel_admin_interface_delete(self):
        ...
        if isinstance(select_data, tuple):
            ...
            return
        empleados = self.get_trabajadores_supervisor(int(supervisor_id))
        # print(empleados)
        if empleados != []:
    ...
    def generar_excel(self, id_area):
        print(id_area)
        # data de bienes
        bienes = self.controlador.query_tomar_data_bienes_controlador(id_area)
        ...
        self.messageShow("Se genero un registro de bienes en Excel correctamente")
