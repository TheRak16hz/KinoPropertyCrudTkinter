...
class Model():
    ...
    def admin_query_customThree(self, opc : int, old_info = (), new_info = ()):
        ...
        if len(old_info)>3:
            delete_info = old_info[3]
        elif len(old_info)>=2:
            delete_info = old_info[2]
        print(update_info)
        print(delete_info)
        # imprimiendo valores
        ...
        if opc == 1:
            ...
            query = f"DELETE FROM supervisor_area_kino WHERE id_supervisor = ?"
            fila = self.run_query(query, (delete_info, ), 2)
            print(fila)
            if isinstance(fila, int) == True:
                return fila
            else: return fila
...
# objeto = Model()
# print(objeto.query_tomar_data_bienes(['1', '3']))